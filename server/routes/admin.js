...
const {
    generateActivationKey, computeExpiryDate, safeString,
    generateResetCode, VALID_BUSINESS_TYPES, VALID_INVOICE_TEMPLATES
} = require('../lib/utils');
...
// ============================================================
// v6.8.0: PUT /licenses/:id/business-type
// تغيير نوع النشاط + قالب الفاتورة لترخيص محدد
// ============================================================
router.put('/licenses/:id/business-type', async (req, res) => {
    const id = parseInt(req.params.id);
    const { business_type, invoice_template, reason } = req.body || {};

    // التحقق من صحة المدخلات
    if (!business_type || !invoice_template) {
        return res.status(400).json({ error: 'missing_fields', message: 'business_type و invoice_template مطلوبان' });
    }
    if (!VALID_BUSINESS_TYPES.has(String(business_type))) {
        return res.status(400).json({ error: 'invalid_business_type', value: business_type });
    }
    if (!VALID_INVOICE_TEMPLATES.has(String(invoice_template))) {
        return res.status(400).json({ error: 'invalid_invoice_template', value: invoice_template });
    }
    ...
    try {
        ...
        const oldBusiness = lic.business_type;
        const oldTemplate = lic.invoice_template;

        await query(
            "UPDATE ts_licenses SET business_type=$1, invoice_template=$2, updated_at=NOW() WHERE id=$3",
            [String(business_type), String(invoice_template), id]
        );

        // تسجيل الحدث في سجل الأحداث
        try {
            await query(`
                INSERT INTO ts_license_events (license_id, event_type, machine_id, details)
                VALUES ($1, 'business_type_changed', $2, $3)
            `, [
                id,
                lic.machine_id || null,
                JSON.stringify({
                    old_business_type: oldBusiness,
                    new_business_type: business_type,
                    old_invoice_template: oldTemplate,
                    new_invoice_template: invoice_template,
                    reason: safeString(reason, 500) || null
                }).substring(0, 1000)
            ]);
        } catch (evErr) {
            console.warn('[business-type change] log event failed:', evErr.message);
        }
        ...
        return res.json({
            ...
            invoice_template: String(invoice_template),
            old_business_type: oldBusiness,
            old_invoice_template: oldTemplate,
            note: 'سيتم تطبيق التغيير على الجهاز عند أول heartbeat (تجديد التوكن).'
        });
    } catch (e) {
        console.error('[business-type change]', e);
        return res.status(500).json({ error: e.message });
    }
});

// ============================================================
// v6.8.0: POST /licenses/:id/generate-reset-code
// توليد كود إعادة تعيين كلمة المرور — صالح 24 ساعة
// ============================================================
router.post('/licenses/:id/generate-reset-code', async (req, res) => {
    const id = parseInt(req.params.id);
    const { ttl_hours } = req.body || {};
    const ttl = Math.max(1, Math.min(Number(ttl_hours) || 24, 168)); // 1h..168h (7 أيام)

    try {
        const licR = await query("SELECT id, machine_id, client_id, status FROM ts_licenses WHERE id=$1", [id]);
        const lic = licR.rows[0];
        if (!lic) return res.status(404).json({ error: 'not_found' });
        if (lic.status === 'revoked') {
            return res.status(403).json({ error: 'license_revoked', message: 'لا يمكن توليد كود لترخيص ملغى' });
        }

        // توليد كود فريد (نتحقق من عدم التكرار)
        let code = null;
        for (let i = 0; i < 20; i++) {
            const candidate = generateResetCode();
            const dup = await query(
                "SELECT id FROM ts_licenses WHERE reset_code=$1 AND reset_code_used_at IS NULL AND reset_code_expires_at > NOW()",
                [candidate]
            );
            if (dup.rows.length === 0) { code = candidate; break; }
        }
        ...
        await query(`
            UPDATE ts_licenses
            SET reset_code = $1,
                reset_code_generated_at = $2,
                reset_code_expires_at = $3,
                reset_code_used_at = NULL,
                updated_at = NOW()
            WHERE id = $4
        `, [code, now, expiresAt, id]);

        // تسجيل الحدث
        try {
            await query(`
                INSERT INTO ts_license_events (license_id, event_type, machine_id, details)
                VALUES ($1, 'reset_code_generated', $2, $3)
            `, [
                id,
                lic.machine_id || null,
                JSON.stringify({
        ...
        return res.json({
            ...
            expires_at: expiresAt.toISOString(),
            ttl_hours: ttl,
            note: 'أعطِ العميل هذا الكود ليدخله في شاشة "نسيت كلمة المرور" داخل التطبيق.'
        });
    } catch (e) {
        console.error('[generate-reset-code]', e);
        return res.status(500).json({ error: e.message });
    }
});
