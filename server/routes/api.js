...
/* ═══════════════════════════════════════════════════════════════════════════
   v6.9.1 FIX (ثغرة أمنية #A1): إزالة السرّ الثابت المكشوف من الكود المصدري.
   السرّ المشترك مع الديسكتوب أصبح إلزامياً عبر متغير البيئة JWT_SECRET.
   بدونه يرفض الخادم البدء (fail-fast) بدل استخدام سرّ معروف يسمح بتزوير التراخيص. */
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET || String(JWT_SECRET).trim().length < 16) {
    console.error('[FATAL] JWT_SECRET غير مضبوط أو قصير جداً (<16 حرفاً). أضِفه في متغيرات البيئة على Render. توقف الخادم.');
    process.exit(1);
}
...
// ============================================================
// v6.8.0: POST /api/verify-reset-token - تحقق من كود إعادة تعيين كلمة المرور
// - يتحقق من الكود مطابقاً مع machine_id للترخيص
// - إذا صالح: يعلّم الكود كمستخدم ويرجع temp_password (كلمة مرور مؤقتة)
// - العميل الديسكتوب يستبدل password_hash لمستخدم admin بهذه الكلمة المؤقتة
//
// المسار موحّد مع lib/license.js:verifyResetToken() في العميل الديسكتوب.
// يقبل reset_token (v5.7.6 backward-compat) أو reset_code (v6.8.0).
// ============================================================
router.post('/verify-reset-token', async (req, res) => {
    // v5.7.6 كان يستخدم reset_token، v6.8.0 يستخدم reset_code — نقبل الاثنين
    const body = req.body || {};
    const rawCode = body.reset_token || body.reset_code || '';
    const { machine_id } = body;
    if (!rawCode || !machine_id) {
        return res.status(400).json({ error: 'missing_fields', ok: false, message: 'reset_token/reset_code و machine_id مطلوبان' });
    }

    try {
        const code = String(rawCode).trim().toUpperCase();
        const midStr = String(machine_id);

        // البحث عن ترخيص بهذا الكود ولنفس machine_id
        const licR = await query(`
            SELECT id, machine_id, client_id, business_type, status,
                   reset_code, reset_code_expires_at, reset_code_used_at, reset_code_generated_at
            FROM ts_licenses
            WHERE UPPER(reset_code) = $1 AND machine_id = $2
            LIMIT 1
        `, [code, midStr]);
        const lic = licR.rows[0];

        if (!lic) {
            await logEvent(null, 'reset_code_failed', req, {
                reason: 'not_found',
                code_prefix: code.substring(0, 4) + '****',
                machine_id: midStr
            });
            return res.status(404).json({ ok: false, error: 'invalid_token', message: 'الكود غير صحيح أو لا يطابق هذا الجهاز.' });
        }

        // هل تم استخدامه من قبل؟
        if (lic.reset_code_used_at) {
            await logEvent(lic.id, 'reset_code_failed', req, {
                reason: 'already_used',
                used_at: toIso(lic.reset_code_used_at)
            });
            return res.status(403).json({ ok: false, error: 'invalid_token', message: 'هذا الكود مُستخدم مسبقاً.' });
        }

        // هل انتهت صلاحيته؟
        if (lic.reset_code_expires_at && new Date(lic.reset_code_expires_at).getTime() < Date.now()) {
            await logEvent(lic.id, 'reset_code_failed', req, {
                reason: 'expired',
                expired_at: toIso(lic.reset_code_expires_at)
            });
            return res.status(403).json({ ok: false, error: 'expired', message: 'انتهت صلاحية الكود. اطلب كوداً جديداً.' });
        }

        // الترخيص ملغى؟
        if (lic.status === 'revoked') {
            return res.status(403).json({ ok: false, error: 'license_revoked', message: 'الترخيص ملغى.' });
        }

        // ✅ ناجح — علّم الكود كمستخدم وأصدر كلمة مرور مؤقتة عشوائية آمنة
        const nowDate = new Date();
        await query(
            "UPDATE ts_licenses SET reset_code_used_at=$1, updated_at=NOW() WHERE id=$2",
            [nowDate, lic.id]
        );

        // توليد كلمة مرور مؤقتة قوية (12 حرفاً: حرف كبير + صغير + رقم + رمز)
        const tempPassword = generateTempPassword();

        await logEvent(lic.id, 'reset_code_verified', req, {
            code_prefix: code.substring(0, 4) + '****',
            machine_id: midStr
        });
        ...
        return res.json({
            ...
            must_change: true,
            license_id: lic.id,
            message: 'تم التحقق من الكود بنجاح. تم إصدار كلمة مرور مؤقتة — يجب تغييرها فور تسجيل الدخول.'
        });
    } catch (e) {
...
// v6.8.0: توليد كلمة مرور مؤقتة قوية (12 حرفاً)
function generateTempPassword() {
    ...
    let out = '';
    // ضمان وجود واحد من كل فئة (indexes 0..3)
    out += upper[buf[0] % upper.length];
    out += lower[buf[1] % lower.length];
    out += digit[buf[2] % digit.length];
    out += sym[buf[3] % sym.length];
    for (let i = 4; i < 12; i++) out += all[buf[i] % all.length];
    // خلط بسيط
    return out.split('').sort(() => (crypto.randomBytes(1)[0] & 1) ? 1 : -1).join('');
}
